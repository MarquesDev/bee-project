...
const getUrl = stage => {
  if (stage === 'production') return `www.allo-maya.fr`;
  throw new Error(
...
const execWithoutCompression = stage => () =>
  new Promise((resolve, reject) => {
    ...
    const command = `aws s3 sync ./public s3://${url} --acl public-read --region=eu-west-3 --exclude '*.js' --exclude '*.html' --exclude '*.css'`;
...
const execWithCompression = stage => () =>
  new Promise((resolve, reject) => {
    const url = getUrl(stage);
    const command = `aws s3 sync ./public s3://${url} --acl public-read --region=eu-west-3 --exclude '*' --include '*.js' --include '*.css' --exclude '*-worker.js' --content-encoding=gzip`;
    return exec(command, (error, stdout, stderr) => {
      if (error) return reject(error);
      return resolve();
    });
...
const execWithCompressionHtml = stage => () =>
  new Promise((resolve, reject) => {
    const url = getUrl(stage);
    const command = `aws s3 sync ./public s3://${url} --acl public-read --region=eu-west-3 --exclude '*' --include '*.html' --content-encoding=gzip --cache-control max-age=0`;
    return exec(command, (error, stdout, stderr) => {
      if (error) return reject(error);
      return resolve();
    });
